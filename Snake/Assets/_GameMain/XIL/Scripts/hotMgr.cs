...
namespace wxb
{
	...
	public static class hotMgr
	{
		...
		static public async UniTask InitHotModule()
		{
			...
			try
			{
				var dll = Loader.LoadAssetAsync<UnityEngine.TextAsset>("DyncDll.dll").ToMyUnitask();
				var pdb = Loader.LoadAssetAsync<UnityEngine.TextAsset>("DyncDll.pdb").ToMyUnitask();
				var (r1, r2) = await UniTask.WhenAll(dll, pdb);

				var dllStream = new MemoryStream(r1.bytes);
				var pdbStream = new MemoryStream(r2.bytes);
