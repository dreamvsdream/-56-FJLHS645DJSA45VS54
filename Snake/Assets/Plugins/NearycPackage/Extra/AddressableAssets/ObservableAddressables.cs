...
#else
		public static System.IObservable<GameObject> ResInstantiate(object key, Transform parent = null, Action<AsyncOperation> onCompleted = null, bool isWorldSpace = false)

		{
			// Addressables.Instantiate<T>(key)
			return Observable.FromCoroutine<GameObject>((observer, token) =>
			{
				return ResInstantiateCore(key, observer, token, parent, onCompleted, isWorldSpace);
			});
		}
		private static IEnumerator ResInstantiateCore(
			object key, IObserver<GameObject> observer, CancellationToken token, Transform parent = null, Action<AsyncOperation> onCompleted = null, bool isWorldSpace = false)

		{
			var aop = Resources.LoadAsync<GameObject>(key.ToString());
			if (onCompleted != null)
			{
				aop.completed += onCompleted;
			}
			...
			if (token.IsCancellationRequested) yield break;
			var go = GameObject.Instantiate(aop.asset as GameObject,parent);
			observer.OnNext(go);
			observer.OnCompleted();
		}
#endif
