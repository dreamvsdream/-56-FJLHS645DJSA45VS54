...
namespace wxb.IL.Editor
{
    static class ILTools
    {
        ...
        static void AddMonoMarco()
        {
            ...
            AddMarco("Assets/_GameMain/XIL/ILSource/", 
                (file) => 
                {
                    if (file.StartsWith("Assets/_GameMain/XIL/ILSource/ILRuntime/"))
                    {
                        if (file.StartsWith("Assets/_GameMain/XIL/ILSource/ILRuntime/Runtime/CLRBinding/"))
                            return ilruntime_editor;
                        return ilruntime;
                    }
                    else if (file.StartsWith("Assets/_GameMain/XIL/ILSource/Mono.Cecil/"))
                    {
                        if (file.StartsWith("Assets/_GameMain/XIL/ILSource/Mono.Cecil/symbols/mdb"))
                            return mono_mdb;
                        else if (file.StartsWith("Assets/_GameMain/XIL/ILSource/Mono.Cecil/symbols/pdb"))
                            return mono_pdb;
                        return mono;
                    }
